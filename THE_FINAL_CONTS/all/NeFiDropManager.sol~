...
function getAllOld(uint256[] memory addsNums,uint256[] memory nameNums,string[] memory _names) external{
	uint256 count = Zero;
	for(uint i=0;i<addsNums.length;i++){
		address _account = protoMGR.getProtoAddress(i);
		airdrop[_account].dropped = dropMGR.EXTgetdropped(_account);
		for(uint j = 0;j<nameNums[i];j++){
			if(feeMGR.EXTnameExists(_account,_names[count])==false){
				airdrop[_account].claimed +=1;
				string memory _name = _names[count];
			}
			count += One;
		}
	}
	protoMGR.getProtoStarsLength()
}
