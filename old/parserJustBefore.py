...
def bulkUpdateVarisInfo():
    ...
    varsLs = [file,name,currFileDir,currFilePath,originalFilePath,originalFileDir,currProjectDir,currFileDir,currFilePath]
            ...
            if pra in track:
                ...
                if prag in track[pra]:
                    ...
                    for k in range(0,len(nam)):
                        if ls[i] in track[pra][prag][nam[k]]:
                                lines[currLine] = reader(track[pra][prag][nam[k]][ls[i]])+'\n'+lines[currLine]
                                pen(linesToString(lines),currFilePath)
...
def importCheck(name,st):
    ...
    for i in range(0,len(pragFold)):
        ...
        if st in namefold:
            stPath = createPath(namePath,st)
            stFold = os.listdir(stPath)
            if name+'.sol' in stFold:
                finalPath = createPath(stPath,name+'.sol')
                lines[currLine] = reader(finalPath)+'\n'+lines[currLine]
                pen(linesToString(lines),currFilePath)
                changeGlob('lines',readLines(currFilePath))
                changeGlob('lineTrack',{})
                changeGlob('currLine',0)
...
def saveSect(st,i):
    ...
    print(stopWhenClosed(linesToString(lines[i:]),['{','}']),lic,pragma)
...
def allLinesParse():
    ...
    for i in range(0,len(lines)):
        ...
        if isCurrent(x,'SPDX-License-Identifier') == True:
            SPDXLines(x,i)
        elif isCurrent(x,'pragma solidity') == True:
            getPragLines(x,i)
        elif isCurrent(x,'library') == True:
            changeGlob('currLine',i)
            queryLibraries(x,i)
            saveSect('library',i)
        elif isCurrent(x,'using') == True:
            queryLibrariesUse(x,i)
        elif isCurrent(x,'contract') == True:
            changeGlob('currLine',i)
            queryContracts(x,i)
            saveSect('contract',i)
        elif isCurrent(x,'abstract contract') == True:
            changeGlob('currLine',i)
            queryAbstract(x,i)
            saveSect('abstract contract',i)
        elif isCurrent(x,'interface') == True:
            changeGlob('currLine',i)
            queryInterface(x,i)
            saveSect('interface',i)
        elif isCurrent(x,'constructor') == True:
            queryConstructorData(x,i)
        elif isCurrent(x,'function') == True:
            queryFunctions(x,i)
            saveSect('function',i)
...
def queryLibrariesUse(x,i):
        ...
        lineTrack['library'][name]['attributes'][str(i)] = use
...
def queryInterface(x,i):
    ...
    if str(lineTrack['interface']['names'][-1]) not in lineTrack['interface']:
        lineTrack['interface'][str(lineTrack['interface']['names'][-1])] = {'address':"","attributes":{}}
    if ' is ' in x:
                ...
                if '[]' in lineTrack['contract'][contractName]["constructor"]["attributes"][c]:
                    ...
                    for i in range(0,len(constructor)):
                        ...
                        if '=' in constructor[i]:
                            ...
                            if '0x' in eq:
                                lineTrack['contract'][contractName]["constructor"][str(var.replace(' ',''))] = {"address":str(eq.split('0x')[1][:len('0000000000000000000000000000000000000000')])}
...
def getInitialVars():
    ...
    input([lineTrack['contract']['lines'][-1]+1,getJustHigher(lineTrack['contract']['lines'][-1],lineTrack['function']['lines'])])
...
def createFiles():
    ...
    pen(reader('variables/samples/deploy.js').replace('^^0^^','require("@nomiclabs/hardhat-etherscan");').replace('^^1^^',name).replace('^^2^^','').replace('^^3^^',str(lineTrack['contract'][getAny('contract','names')[-1]]["constructor"]['input'])),createPath(currFileDir,name+'_deploy.js'))
...
def deriveCurrentDir():
    changeGlob('currFileDir',existFoldCheck(createPath(currProjectDir,name)))
...
def checkName():
    ...
    lsN = ['input New Name',name]
    if name not in names:
        for i in range(0,len(names)):
            lsN.append(names[i])
        newName = createAsk(lsN,lsN,'looks like there is a discrepency in the naming convention, we recorded '+str(name)+'; please choose the name that does not belong.')
        input(newName)
        ...
        for i in range(0,len(lines)):
            if isCurrent(lines[i],'contract') == True:
                ...
                if newName in lines[i]:
                    lines[i] = lines[i].replace(newName,name)
...
def retrieveFileinfo():
    newFileIno = js_it({"projectDir":projectDir,"originalFilePath":str(originalFilePath),"originalFileDir":str(originalFileDir),"currProjectDir":str(currProjectDir),"currFilePath":str(currFilePath),"file":str(file), "name":str(name),"contract_address":"","api":str(keys()),"pragma":[],"license":"","args":"", "version":"","variableNames":[],'allVarsTrack':{}})
