...
def mains():
    ...
    all = main[network]
    changeGlob('scanners',all["scanners"])
    changeGlob('net',all["net"])
    changeGlob('chain',all["chain"])
    changeGlob('main_tok',all["main_tok"])
    changeGlob('w3',Web3(Web3.HTTPProvider(all["net"])))
...
def boolAsk(x):
    while True:
        ask = input(x)
        if str(ask).lower() in ['y','yes','true','t','']:
            return True
        elif str(ask).lower() in ['n','no','false','f']:
            return False
...
def isCurrent(x,y):
    ...
    if 'constructor' in y:
        ex = '('
    if 'SPDX-License-Identifier' in y:
        ex = ':'
    if x[:len(y)+1] == y+ex:
        return True
    return False
...
def trackEm(ls):
    ...
    track = json.loads(reader_c('variables/pragmas/tracker.json').replace("'",'"'))
...
def queryContracts(x,i):
    ...
    if ' is ' in x:
        lineTrack['contract'][str(getAny('contract','names')[-1])]['attributes'] = checkAtts(lineTrack['contract'][str(getAny('contract','names')[-1])]['attributes'],checkIsList(getAtts(x)))
                    ...
                    for i in range(0,len(constructor)):
                        ...
                        if lineTrack['contract'][contractName]["constructor"]["constVariables"][c]+'[' in constructor[i]:
                            ...
                            if isInt(constInt) == True:
                                higher = getHigher(int(constInt)+1,higher)
                            else:
...
def getContName(x):
    info = getSourceCodeInfo(str(x))
    return info['result'][int(0)]['ContractName']
def askAdd(add,na,na2):
    if boolAsk('looks like the contract address '+str(add)+' is deployed on the network under the name '+str(getContName(add))+' is this the correct one to associate with  '+str(na)+' - '+str(na2) +'?') == False:
        return False
    return add
def inputAssociatedContract(na,na2):
    add = '0x'
    while addressVeri(add) == False:
        add = input('please input the address youd like to use to associate with '+str(na)+' - '+str(na2) +' (input * for burn address):')
        if add == '*':
            add = '0x0000000000000000000000000000000000000000'
    return add
def addressVeri(x):
    if len(str(x)) == len('0x0000000000000000000000000000000000000000'):
        info = getSourceCodeInfo(str(x))
        if info['status'] != 0:
            return info
    return False
def choosePrevDeploy(add):
    if len(varisInfo['names']) >= 2:
        while True:
            newDep = createAsk(varisInfo['names'][:-1],varisInfo['names'][:-1],'which contract would you like to choose?')
            if newDep == 'back':
                return add
            elif boolAsk("you're sure you want to use "+ str(newDep)+'?') == True:
                lineTrack[ca]['names']['address'] = json.loads(str(reader(createPath(varisInfo['currProjectDir'][findIt(newDep,varisInfo['names'])],'info.json'))))['contractAddress']
                return lineTrack[ca]['names']['address']
    else:
        print('you have no previous deploys associated with this project')
        return add
def addressAssociatioin(originalAdd,na,na2):
    curr = originalAdd
    while askAdd(str(curr),na,na2) == False:
        curr = createAsk(['input an existing contract address?','keep the current address of '+str(originalAdd)+'?','utilize a contract from this deployment package?'],['a','b','c'],'what would you like to do?')
        while addressVeri(curr) == False:
            if curr == 'a':
                curr = inputAssociatedContract(na,na2)
            elif curr == 'b':
                curr = str(originalAdd)
            elif curr == 'c':
               curr = choosePrevDeploy(curr)
    return curr
def isChange(x):
    if x == '0x00000000000000000000000000000000000DEAD':
        return True
    return False
def checkConstructorArgs(x):
    if '[' in x:
        newInt = int(x.split('[')[1].split(']')[0])
        x = x.split('[')[0]
    na = lineTrack['contract']['names']
    for i in range(0,len(na)):
        contName = lineTrack['contract'][na[i]]
        if 'constructor' in contName:
            contVars = contName['constructor']['constVariables']
            if x in contVars:
                k = findIt(x,contVars)
                depVars = contName['constructor']['deployerVars']
                if contVars[k] not in depVars:
                    depVars[contVars[k]] = []
                    fileInfo["args"].append([])
                for c in range(len(contName['constructor']['input']),int(contName['constructor']['attributes'][k].split('[')[1].split(']')[0])):
                    contName['constructor']['input'].append('0x0000000000000000000000000000000000000000')
                    depVars[contVars[k]].append(x+'['+str(int(c)+1)+']')
                    fileInfo["args"][k].append(x+'['+str(int(c)+1)+']')
                return na[i],depVars[contVars[k]],k,newInt,x
def eatInitVar(x):
...
def getInitialVars():
    lineTrack['variables'] = {'declared':[],'headers':[],'rndm':[],'address':{"list":[],"single":[]},'uint':{"list":[],"single":[]},'string':{"list":[],"single":[]},'bool':{"list":[],"single":[]},'bytes':{"list":[],"single":[]},'mapping':{}}

    kk = lineTrack['contract']['lines'][-1]+1
    while len(ListInString(str(lines[kk]),['constructor','function'])) <1:
        n = lines[kk]
        if '=' in n:
            n = n.split('=')[0]
        n = eatAllMod(n,[' ',';','\n','\t'])
        curTy = isCurrentTypeFor(n)
        if curTy != False:
            if curTy[1] == 'mapping':
                lineTrack['variables'][curTy[0]][lsOrSing(curTy[1])].append({"type":str(curTy[1]),"visibility":str(isMid(n)),"modifier":str(isMod(n)),"name":eatAllMod(n.replace(curTy[1],'').replace(str(isMid(n)),'').replace(str(isMod(n)),''),[' ',';','\n','\t'])})
...
def makeConstructors():
    na = getAny('contract','names')[-1]
    n = lineTrack['contract'][na]["constructor"]["attributes"]
    units = lineTrack['contract'][na]["constructor"]["input"]
    lsConsts= json.loads(existWriteFileRead({},createPath(currFileDir,'constVals.sol')).replace("'",'"'))
    lsVerify = []
    if len(n) > len(units):
        for i in range(len(units),len(n)):
            if '[' in n[i]:
                if n[i].split('[')[1].split(']')[0] == 'usr':
                    ask = ''
                    while isInt(ask) == False:
                        ask = input('looks as if the constructor value is dependant on how many youd like to input, please enter the number of '+str(n[i].split('[')[0])+' that youd like to input ( enter * for the default number (1)')
                        if ask == '*':
                            ask = 1 
                        print('looks like you didnt enter an integer input, please try again,( enter * for the default number (1)')
                    n[i] =  n[i].split('[')[1]+'['+str(ask)+']'
                lsN = []
...
def flatten_it():
    ...
    print(newFlat,na,scriptPath,scriptConst)
...
def getSourceCodeInfo(x):
    key = keys()
    return sites('https://'+str(scanners)+'/api?module=contract&action=getsourcecode&address='+str(x)+'&apikey='+str(key))
def getCalls(at):
...
def newFileInfo():
    ...
    changeGlob('fileInfo',js_it({"projectDir":projectDir,"originalFilePath":str(originalFilePath),"originalFileDir":str(originalFileDir),"currProjectDir":str(currProjectDir),"currFilePath":str(currFilePath),"file":str(file), "name":str(fileName),"contract_address":"","api":str(keys()),"pragma":[],"license":"","args":[], "version":"","variableNames":[],'allVarsTrack':{}}))
#retrieveInfo----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------
...
def retrieveFileinfo():
    ...
    changeGlob('fileInfo',js_it(existFileMakeSpec(newFileIno,infoFilePath)))
def runIt():
    mains()
    initialize()
    createFolders()
...
def compileImports():
    ...
    for i in range(0,len(ls)):
        ...
        changeGlob('fileInfo',newFileIno)
        changeGlob('currHead',None)
        deriveCurrentDir()
