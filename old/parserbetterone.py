...
def sendIfs(x,i):
    ...
    saveLst = ['interface','contract','library','abstract contract']
    for k in range(0,len(heads)):
        if isCurrent(x,heads[k]) == True:
            if heads[k] in saveLst:
                changeGlob('currHead',heads[k])
...
def allLinesParse():
    ...
    for i in range(0,len(lines)):
        x = eatAllMod(lines[i],['\n','\t',' ','/'])
        if walked == 'go' and currHead != None:
            if currHead in ['interface','contract','library','abstract contract'] and lineTrack[currHead]['names'][-1] == varisInfo['names'][-1]:
                getInitialVars()
                walked = 'no'
        sendIfs(x,i)
...
def countSpecInLsSpec(ls,y):
    ...
    return False
def getBraks(ls,y):
    while countSpecInLsSpec(ls,y[0]) == True:
        ls = checkIsList(ls)
        lsN = []
        for i in range(0,len(ls)):
            if '(' in x:
                lsN.append(stopWhenClosed(linesToString(ls[i]),[y[0],y[1]]))
            else:
                lsN.append(ls[i])
        ls = lsN
    return ls
...
def queryFunctions(x,i):
    ...
    if lineTrack[currHead]['names'][-1] == varisInfo['names'][-1]:
        full = checkIsList(stopWhenClosed(linesToString(lines[i:]),['{','}']).split('\n')[1:-1])
        full = checkIsList(getBraks(full,['(',')']))
        for i in range(0,len(full)):
            ful = checkIsList(eatAllMod(full[i],['\n','\t',' ','/',',',';']).split(' '))
            heas = ['interface','abstract contract','library','contract']
            for j in range(0,len(heas)):
                hea = heas[j]
                for cc in range(0,len(ful)):
                    ffu = str(ful[cc])
                    if ffu in lineTrack[hea]['names']:
                        print(lineTrack[hea]['names'])
                        if str(ffu) not in lineTrack['variables']['declared']:
                            lineTrack['variables']['declared'].append(ffu)
                    else:
                        if ffu not in lineTrack['variables']['undeclared']:
                            lineTrack['variables']['undeclared'].append(ffu)
...
def queryLibrariesUse(x,i):
        ...
        if 'library' not in lineTrack[currHead][lineTrack[currHead]['names'][-1]]['attributes']:
            lineTrack[currHead][lineTrack[currHead]['names'][-1]]['attributes']['library'] = []
        lineTrack[currHead][lineTrack[currHead]['names'][-1]]['attributes']['library'].append(name)
...
def queryInterface(x,i):
    ...
    if str(lineTrack['interface']['names'][-1]) not in lineTrack['interface']:
        lineTrack['interface'][str(lineTrack['interface']['names'][-1])] = {'address':[],"attributes":{},"undeclared":"1","name":[]}
        ...
        if len(lineTrack['constructor']['lines'])>0:
            if 'constructor(' in lines[getAny('constructor','lines')[-1]]:
                ...
                for i in range(0,len(const)):
                    con = const[i].split(' ')
                    if '[' in con[0]:
                        if con[0].split('[')[1].split(']')[0] == '':
                            con[0] = con[0].split(']')[0]+str(1)+']'
                    if len(con) == 2:
                        ...
                        for kk in range(0,len(ls)):
                            ...
                            if len(abstractLs) !=None:
                                for k in range(0,len(abstractLs)):
                                    ...
                                    lineTrack[ls[kk]][abstractLs[k]]['name'].append(str(eatAllMod(spl[0].split('=')[0].replace(' ','').replace('\n','').replace('\t',''),['\n','\t',' ','/',','])))
                                    lineTrack[ls[kk]][abstractLs[k]]['address'].append(spl[1].split(')')[0].replace('\n','').replace('\t',''))
...
def eatInitVar(x):
    ...
    if len(z) < 1:
        return False
    return z
def catchInSpaces(x,y):
    return x.split(' '+y+x.split(y)[1].split(' ')[0]+' ')
def isCurrentType(x,y):
    if x[:len(y)] == y:
        return True
    return False
def isCurrentTypeFor(x):
    typ = ['address','uint','string','bool','struct']
    for i in range(0,len(typ)):
        if isCurrentType(x,typ[i]) == True:
            return typ[i],x.split(' ')[0]
    return False
def isMid(n):
    mid = ["public","private","external","internal"]
    for i in range(0,len(mid)):
        if mid[i] in n:
            return mid[i]
    return mid[0]
def isMod(x):
    mod = ["pure","view","payable","constant","immutable","anonymous","indexed","virtual","override"]
...
def mergeString(ls):
    ...
    for i in range(0,len(ls)):
        if ls[i] != '':
            z = z + str(ls[i])+' '
    return eatAllMod(z,[' '])
def searchDeclarations(na):
    typ = ['address','uint','string','bool','mapping']
    ls = ['list','string']
    for i in range(0,len(typ)):
        now = lineTrack['variables'][typ[i]]
        lss = lsInJs(now,ls)
        if len(lss) != 0:
            for c in range(0,len(lss)):
                if lss[c] in now:
                    nowSpec = now[lss[c]]
                    for k in range(0,len(nowSpec)):
                        if nowSpec[k]["name"] == na:
                            return mergeString([typ[i],lss[c],nowSpec[k],na]),nowSpec[k]
        else:
            for k in range(0,len(now)):
                if nowSpec[k]["name"] == na:
                    return  mergeString([typ[i],nowSpec[k],na]),nowSpec[k]
    return False
    
def declared(x):
    if thenAppendBool(x[-1]["name"],lineTrack['variables']['declared']) == False:
        lineTrack['variables']['declared'].append(x[-1]["name"])
        return False
    else:
        return searchDeclarations(x[-1]["name"])
def modifyDec(dec):
    var,js = dec[0],dec[1]
    ask = createAsk(['modify','delete'],['modify','delete'],'looks as if youve already claimed '+str(n) + ' as '+str(dec)+' would you like to modify or delete it?')
    if ask == 'modify':
        lsN = []
        decLs = ["visibility","types","modifier","name"]
        ...
        while ex == False:
            modIt = True
            while modit == True:
                mod = createAsk(lsN,lsN,'what would you like to modify '+var+' to?(type exit to exit)')
                if mod == 'types':
                    js[mod] = createAsk(['address','uint','string','bool','mapping'],['address','uint','string','bool','mapping'],'currently the '+mod+' is '+js[mod]+' what would you like to change it to?')
                elif mod == 'visibility':
                    js[mod] = createAsk(["public","private","external","internal"],["public","private","external","internal"],'currently the '+mod+' is '+js[mod]+' what would you like to change it to?')
                elif mod == "modifier":
                    js[mod] = createAsk(["pure","view","payable","constant","immutable","anonymous","indexed","virtual","override"],["pure","view","payable","constant","immutable","anonymous","indexed","virtual","override"],'currently the '+mod+' is is '+js[mod]+' what would you like to change it to?')
                elif mod == "name":
                    js[mod] = inp = input('what would you like to change the name too?')
                if mod == 'exit':
                    modIt == False
            if boolAsk('are you sure you want to change the name to '+str(mergeString([js["visibility"],js["types"],js["modifier"],js["name"]]))+'?') == True:
                ex = True
    elif ask == 'delete':
        #dele = createAsk(['address','uint'
        if boolAsk('are you sure you want to delete the variable to '+str(inp)+'?') == True:
            print('i got nothinmg bro')

def getInitialVarsSpec(x,y):
        curTy = isCurrentTypeFor(x)
        if curTy[1] == 'mapping':
            lineTrack['variables'][y][lsOrSing(curTy[1])].append({"type":str(curTy[1]),"visibility":str(isMid(n)),"modifier":str(isMod(n)),"name":eatAllMod(n.replace(curTy[1],'').replace(str(isMid(n)),'').replace(str(isMod(n)),''),[' ',';','\n','\t'])})
            dec = declared(lineTrack['variables'][curTy[0]][lsOrSing(curTy[1])])
        else:
            lineTrack['variables'][y][lsOrSing(curTy[1])].append({"type":str(curTy[1]),"visibility":str(isMid(n)),"modifier":str(isMod(n)),"name":eatAllMod(n.replace(curTy[1],'').replace(str(isMid(n)),'').replace(str(isMod(n)),''),[' ',';','\n','\t'])})
            dec = declared(lineTrack['variables'][y][lsOrSing(curTy[1])])
        if dec != False:
            modifyDec(dec)    
        else:
             lineTrack['variables']['headers'].append({"type":str(importCheck(['interface','abstract contract','library','contract'],n)),"visibility":str(isMid(n)),"modifier":str(isMod(n)),"name":eatAllMod(n.replace(str(curTy),'').replace(str(isMid(n)),'').replace(str(isMod(n)),''),[' ',';','\n','\t'])})
...
def getInitialVars():
    ...
    while isAboveZero(constVars) == True and 'constructor' not in constVars[0]:
        n = eatAllMod(constVars[0],['\n','\t',' ','/',',',';'])
        if 'struct' in n and isAboveZero(constVars) == True:
            na = n.split('struct')[1]
            if '{' in na:
                na = eatAllMod(na.split('{')[0],['\n','\t',' ','/',',',';'])
            if na not in lineTrack['variables']['struct']:
                lineTrack['variables']['struct'][na] = []
            structs = stopWhenClosed(linesToString(constVars),['{','}']).split('\n')
            for j in range(1,len(structs)-1):
                print(j)
                stru = eatAllMod(structs[j],['\n','\t',' ','/',',',';'])
                print(stru,isCurrentTypeFor(stru))
                lineTrack['variables']['struct'][na].append({"type":str(isCurrentTypeFor(stru)[0]),"visibility":str(isMid(stru)),"modifier":str(isMod(stru)),"name":eatAllMod(stru.replace(str(isMid(stru)),'').replace(str(isMod(stru)),'').replace(str(isCurrentTypeFor(stru)[1]),''),[' ',';','\n','\t'])})
                dec = declared(lineTrack['variables']['struct'][na])
            lineTrack['variables']['struct']
        ...
        if '=' in n:
            ...
            elif getinterAbsConts(nA,nB) != False:
                na,dec,add = getinterAbsConts(nA,nB)
                heas = ['interface','abstract contract','library','contract']
                for i in range(0,len(heas)):
                    he = lineTrack[heas[i]]
                    if dec in he['names']:
                        he[dec]['name'] = ThenAppend(he[dec]['name'],na)
                        he[dec]['address'] = ThenAppend(he[dec]['address'],add)
            else:
            ...
            if inpu == False:
                for i in range(0,len(n)):
                    lst = list_files(imports)
                    heas = ['interface','abstract contract','library','contract']
                    if n[i] in lst:
                        li = reader(createPath(imports,n[i])).split('\n')
                        for k in range(0,len(li)):
                            n = eatAllMod(li[k],['\n','\t',' ','/',',',';'])
                            for j in range(0,len(heas)):
                                if len(n[:len(heas[j])]) == heas[j]:
                                    heas[j],n[i]
                                    if n[i] not in lineTrack[heas[j]]['names']:
                                         lineTrack[heas[j]]['names'].append(str(n[i]))
                                    if n[0] not in lineTrack[heas[j]]:
                                         lineTrack[heas[j]][n[0]] = {'address':[],"attributes":{},"undeclared":"0","name":[]}
                                    if n[-1] not in lineTrack[heas[j]][n[0]]['name']:
                                        lineTrack[heas[j]][n[0]]['name'].append(n[-1])
                                        ge = getAtts(n)
                                        for kk in range(0,len(ge)):
                                            gee = ge[kk]
                                            for cc in range(0,len(heas)):
                                                if gee in lineTrack[heas[cc]]['names']:
                                                    if heas[cc] not in lineTrack[heas[j]][n[0]]['attributes']:
                                                        lineTrack[heas[j]][n[0]]['attributes'][heas[cc]] = []
                                                    lineTrack[heas[j]][n[0]]['attributes'][heas[cc]].append(gee)

            elif n[0] in lineTrack['abstract contract']['names']:
                lineTrack['abstract contract'][str(n[0])]['name'].append(str(n[-1]))
            elif n[0] in lineTrack['interface']['names']:
                lineTrack['interface'][str(n[0])]['name'].append(str(n[-1]))
            else:    
                lineTrack['variables']['rndm'].append(n)
...
def chmodIt(x):
    ...
    return 'sh '+str(x)
def flatten_it():
    outFolder = varisInfo['currFilePaths'][-1]
...
def getattAtts(st,na):
    print(fileInfo['allVarsTrack'],st,na)
    return fileInfo['allVarsTrack'][st][na]['attributes']
...
def attsLsatts(ls,ls2):
    ...
    if len(ls) == k:
        return True
    return False
def getAttsAll(st,na,fi):
    input([st,na,fi])
    ls = reader(createPath(imports,fi)).split('\n')
    lsN = []
    for i in range(0,len(ls)):
        n = eatAllMod(ls[i],['\n','\t',' ','/',',',';'])
        if n[:len(st)] == st:
            print(n)
            if ' is ' in n:
                atts = n.split(' is ')[1].split('{')[0]
                lsA = checkIsList(atts.replace(' ','').split(','))
                for ii in range(0,len(lsA)):
                    if lsA[ii] not in lsN and lsA[ii] != '':
                        lsN.append(lsA[ii])
        if 'using ' in n:
            lsN.append(n.split('using ')[1].split(' ')[0])
        heads = ['abstract contract','contract','interface']
        for k in range(0,len(heads)):
            na = fileInfo['allVarsTrack'][heads[k]]['names']
            for c in range(0,len(na)):
                if n[:len(na[c])] == na[c] and na[c] not in lsN:
                    lsN.append(na[c])
    return lsN
def getallAtts():
    js = {}
    ...
    for i in range(0,len(lis)):
        now = lis[i]
        nowName = now.split('.sol')[0]
        js[now] = []
        for k in range(0,len(call)):
            if now in fileInfo['allVarsTrack'][call[k]]:
                js[now] = js[now] + getAttsAll(call[k],now,now+'.sol')
    return js
def removeLS(ls,ls2):
    lsN = []
...
def compilerneeds2():
    ...
    for i in range(0,len(lis)):
        if len(js[lis[i]]) == 0:
            lsN.append(lis[i])
    while len(lis) >0:
        lis = removeLS(lsN,lis)
        for i in range(0,len(lis)):
            if attsLsatts(js[lis[i]],lsN) == True:
                lsN.append(lis[i])
        print(lsN)
    alls = ''
    for i in range(0,len(lsN)):
        alls = alls + '\n'+reader(createPath(imports,lsN[i]+'.sol'))
    pen(alls,createPath(imports,'alls.txt'))
    pen(alls,currFilePath)
#thegoodAttributes------------------------------------------------------------------------------------------------------------------------------------------------------------------------        
def bad():
    ...
    for i in range(0,len(imps)):
        ...
        for k in range(0,len(call)):
            if now in fileInfo['allVarsTrack'][call[k]]:
                print(getNeeds(call[k],now))
                if len(fileInfo['allVarsTrack'][call[k]][now]['attributes']) == 0 and now not in lsN:
                    lsN.append(now)
                    input([now,fileInfo['allVarsTrack'][call[k]][now]['attributes'],lsN])
                    j = findIt(now,nImps)
                    for c in range(0,len(nImps)):
                        if c != j:
                            lsA.append(nImps[c])
                    nImps = lsA
                    lsA = []
    ...
    while len(imps) > 0:
         for i in range(0,len(imps)):
            ...
            for k in range(0,len(call)):
                if now in fileInfo['allVarsTrack'][call[k]]:
                    if attsLsatts(fileInfo['allVarsTrack'][call[k]][now]['attributes'],lsN) == True:
                        lsN.append(now)
                        print(lsN)
                        j = findIt(now,nImps)
                        for c in range(0,len(nImps)):
...
def compileNeeds():
    ...
    while len(imps) !=0:
        ...
        for i in range(0,len(imps)):
            na = imps[i]
            print(getCallType(na,jsCalls),na)
...
def getItGoing():
    ...
    checkName()
    pragmaSpec = str(fileInfo['allVarsTrack']['pragma solidity']['pragma'])
    pen({"declared":lineTrack['variables']['declared'],"undeclared":lineTrack['variables']['undeclared']},createPath(varisInfo['currFileDirs'][-1],'declared.json'))
    lineTrack['variables']['declared'] = []
    lineTrack['variables']['undeclared'] = []
    bulkUpdateVarisInfo()
    print(fileInfo)
    makeConstructors()
    bulkUpdateVarisInfo()
...
def initialize():
    ...
    lineTrack['variables'] = {'declared':[],'undeclared':[],'headers':[],'rndm':[],'struct':{},'address':{"list":[],"single":[]},'uint':{"list":[],"single":[]},'string':{"list":[],"single":[]},'bool':{"list":[],"single":[]},'bytes':{"list":[],"single":[]},'mapping':{}}
