...
def queryFunctions():
    ...
    for i in range(0,len(lines)):
        ...
        if isCurrent(x,'function') == True:
            addLines('function',i)
            lineTrack['function']['names'].append(x.split('function')[1].split('(')[0])
    ...
    for i in range(0,len(lines)):
        ...
        if isCurrent(x,'abstract contract') == True:
            ...
            if str(lineTrack['abstract contract']['names'][-1]) not in lineTrack['abstract contract']:
                lineTrack['abstract contract'][str(getAny('abstract contract','names')[-1])] = {'address':"","attributes":[],"name":""}
            if ' is ' in x:
                atts = x.split(' is ')[1].split('{')[0].replace(' ','')
                if ',' in atts:
                    atts = atts.split(',')
                atts = checkIsList(atts)   
                for k in range(0,len(atts)):
                    lineTrack['abstract contract'][str(getAny('abstract contract','names')[-1])]['attributes'].append(atts[k])
def queryInterface():
    ...
    for i in range(0,len(lines)):
        ...
        if isCurrent(x,'interface') == True:
            ...
            if str(lineTrack['interface']['names'][-1]) not in lineTrack['interface']:
                lineTrack['interface'][str(getAny('interface','names')[-1])] = {"attributes":[],"use":""}
            if ' is ' in x:
                atts = x.split(' is ')[1].split('{')[0].replace(' ','')
                if ',' in atts:
                    atts = atts.split(',')
                atts = checkIsList(atts)   
                for k in range(0,len(atts)):
                    lineTrack['interface'][str(getAny('interface','names')[-1])]['attributes'].append(atts[k])
        ...
        for c in range(0,len(lineTrack['constructor']['constructorVars']["attributes"])):
            if '[]' in lineTrack['constructor']['constructorVars']["attributes"][c]:
                ...
                for i in range(0,len(constructor)):
                    abstractLs = ListInString(constructor[i],lineTrack['abstract contract']['names'])
                    if len(abstractLs) !=None:
                        for k in range(0,len(abstractLs)):
                            spl = constructor[i].split(abstractLs[k]+'(')
                
                            lineTrack['abstract contract'][abstractLs[k]]['name'] = eatAllMod(spl[0].split('=')[0].replace(' ',''),['\n','\t',' ','/',','])
                            lineTrack['abstract contract'][abstractLs[k]]['address'] = spl[1].split(')')[0]
                    if lineTrack['constructor']['constructorVars']["attributes"][c]+'[' in constructor[i]:
                        constInt = constructor[i].split(lineTrack['constructor']['constructorVars']["attributes"][c]+'[')[1].split(']')[0]
...
def getInitialVars():
    ...
    if ifBothOverZero(lineTrack['contract']['lines'],lineTrack['constructor']['lines']) == True:
        constVars = lines[lineTrack['contract']['lines'][-1]+1:lineTrack['constructor']['lines'][-1]-1]
    elif ifBothOverZero(lineTrack['contract']['lines'],getJustHigher(lineTrack['contract']['lines'][-1],lineTrack['function']['lines'])) == True :
    ...
    while isAboveZero(constVars) == True:
        ...
        if '=' in n:
            n = n.split('=')
            nA = eatAllMod(n[0],[' ']).split(' ')
            lsN = ListInString(nA[0],['address','uint','string','bool'])
            nB = eatAllMod(n[-1],[' ']).split(' ')
            print(lsN)
            if len(lsN) != 0:
                 if '[' in nA:
...
def querySPDX():
    lineTrack['SPDX-License-Identifier'] = {"lines":[],'licenses':[],"lic":""}
    for i in range(0,len(lines)):
        x = eatInnerMod(lines[i],['\n','\t',' ','/',','])
        if isCurrent(x,'SPDX-License-Identifier') == True:
            lineTrack['SPDX-License-Identifier']['lines'].append(i)
    ...
    for i in range(0,len(spdxLines)):
        appendAny('SPDX-License-Identifier','licenses',compareList(makeAllComparable(eatAllMod(lines[spdxLines[i]].split('SPDX-License-Identifier:')[1],['\n','\t',' ','/',',']))[0],spdxId))       
    for i in range(0,len(getAny('SPDX-License-Identifier','licenses'))):
...
def getPragAll():
    ...
    if len(lineTrack['pragma solidity']['prags']) !=0:
        fileInfo['pragma'] = lineTrack['pragma solidity']['prags'][0]
def changeSchedule():
...
def determineEndPoint():
    ...
    return allFiles
def getVaris():
    changeGlob('varisPath',createPath(mkDir(currProjectDir),'varis.py'))
    ...
    changeGlob('varisInfo',js_it(existWriteFileRead(varisNew,varisPath)))
    determineEndPoint()
def getCurrentDeploy():
    if list_files(currentFold) == 0:
        print('looks like you dont have a current file')
        determineEndPoint()
    changeGlob('varisInfo',json.loads(reader(createPath(currentFold,'varis.py')).replace("'",'"')))
    changeGlob('currProjectDir',varisInfo['currProjectDir'])
    changeGlob('currFilePath',varisInfo['currFilePath'])
    getVaris()
def createNewDeploy():
    nameAndMakeAll(['projectDir','currProjectDir'],[projectDir,str(input("what would you like to name the new batch?"))])
    changeGlob('last',projectDir)
    getVaris()
def getOldDeploy():
    if list_files(projectDir) == 0:
        print('looks like you dont have a project directory')
        determineEndPoint()
    changeGlob('last',projectDir)
    changeGlob('currProjectDir',getFolder(last))
    getVaris()
def initiateVars():
    nameAndMakeAll(['home','currentFold'],[home,'current'])
    nameAndMakeAll(['home','variablesDir'],[home,'variables'])
    nameAndMakeAll(['home','deployHistDir','projectDir'],[home,'deployHist','projects'])
...
def firstRun():
    ...
    getPragAll()
    top = ''
    if len(lineTrack['SPDX-License-Identifier']['lic']) != 0:
        top = top + '//SPDX-License-Identifier: '+lineTrack['SPDX-License-Identifier']['lic'][-1]+'\n'
    if len(lineTrack['pragma solidity']['pragma']) != 0:
        top = top + lineTrack['pragma solidity']['pragma'][-1]
    lsN = [top]
    for i in range(0,len(lines)):
        if len(eatAllMod(lines[i],['','\t',' ','\n'])) != 0 and i not in lineTrack['SPDX-License-Identifier']['lines'] and i not in lineTrack['pragma solidity']['lines'] and lines[i] != '':
            lsN.append(eatInnerMod(lines[i],['']))
    pen(linesToString(lsN),currFilePath)
    changeGlob('lines',lsN)
def allStart():
    changeGlob('originalFilePath',displayFiles(projectDir))
    ...
    changeGlob('last',originalFileDir)
    ...
    copyFile(originalFilePath,currFileDir)
    firstRun()
    getPragLines()
    getPragAll()
    querySPDX()
