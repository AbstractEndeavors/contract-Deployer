        ...
        for c in range(0,len(namefold)):
            ...
            for k in range(0,len(st)):
                if st[k]+'.sol' in contextfold and st[k]+'.sol' not in os.listdir(imports):
                    ...
                    pen(reader(alls)+'\n'+reader(finalPath),alls)
#                    lines[currLine] = reader(finalPath)+'\n'+lines[currLine]
                    #lines[currLine] = reader(track[pra][prag][nam[k]][var])+'\n'+lines[currLine]
                    #pen(linesToString(lines),fileInfo["currFilePath"])
                    changeGlob('lines',readLinesSpec(reader(alls)+'\n'+linesToString(lines)))
                        #pen(linesToString(lines),currFilePath)
                        #changeGlob('lines',readLines(currFilePath))
                    changeGlob('lineTrack',{})
                    changeGlob('currLine',0)
    ...
    while keepItUp == True:
        ...
        if allFiles == 'rearrange':
            ...
            removeDeploy()
                ...
                #createFiles()
                #chmodIt(createPath(currFileDir,fileName+'_deploy.js'))
                #save = linesToString(lines)
                #for i in range(0,len(os.listdir(imports))):
                #    if os.listdir(imports)[i] != 'alls.txt':
                #        save = save.replace(reader(createPath(imports,os.listdir(imports)[i])),'')
                #newLine = readLinesSpec(save)
                #pen(newLine[0]+'/n'+reader(alls)+linesToString(newLine[1:]),currFilePath)
...
def retrieveFileinfo():
    newFileIno = js_it({"projectDir":projectDir,"originalFilePath":str(originalFilePath),"originalFileDir":str(originalFileDir),"currProjectDir":str(currProjectDir),"":str(currFilePath),"file":str(file), "name":str(fileName),"contract_address":"","api":str(keys()),"pragma":[],"license":"","args":"", "version":"","variableNames":[],'allVarsTrack':{}})
