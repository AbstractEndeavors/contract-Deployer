...
def getInitialVars():
    ...
    while isAboveZero(constVars) == True:
        ...
        if '=' in n:
            ...
            if '(' in nB:
                nNew = nB.split('(')[0]
                ca = ['interface','abstract contract']
                for i in range(0,len(ca)):
                    if nNew in lineTrack[ca[i]]["names"]:
                        add = nB.split('(')[1].split(')')[0]
                        na = lineTrack[ca[i]]['names'][-1]
                        st = ca[i]
                        varNa = lineTrack[ca[i]][nNew]['name']
                        if '0x' in add:
                            lineTrack[ca[i]][nNew]['address'] = addressAssociatioin(add,nNew,varNa)
                        else:
                            na,depVar,k,kInt,constVar = checkConstructorArgs(add)
                            if constVar not in lsConsts:
                                lsConsts[constVar] = []
                            for c in range(len(lsConsts[constVar]),kInt+1):
                                lsConsts[constVar].append('0x0000000000000000000000000000000000000000')
                            adds = addressAssociatioin(lsConsts[constVar][-1],nNew,varNa)
                            fileInfo["args"][k][kInt] = const_it(['address',adds])
                            lsConsts[constVar][kInt] = adds
                            lineTrack['contract'][na]['constructor']['input'][kInt] = adds
                        if lineTrack[ca[i]][nNew]['name'] != nA:
                            if lineTrack[ca[i]][nNew]['name'] == '':
                                lineTrack[ca[i]][nNew]['name'] = nA
                            elif type(lineTrack[ca[i]][nNew]['name']) is not list:
                               lineTrack[ca[i]][nNew]['name'] = [lineTrack[ca[i]][nNew]['name'],nA]
                            elif type(lineTrack[ca[i]][nNew]['name']) is list:
                                lineTrack[ca[i]][nNew]['name'].append(nA)
                pen(lsConsts,createPath(currFileDir,'constVals.sol'))
            elif len(lsN) != 0:
                 if '[' in nA:
...
def compileNeeds():
    ...
    while len(imps) !=0:
        ...
        for i in range(0,len(imps)):
            ...
            if getCallType(na,jsCalls) == None:
                importCheck(jsCalls,na)
            print(na,jsCalls)
